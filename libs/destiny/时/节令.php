...
class 节令 {
	 ...
	 * @return
	 */
	public static function 节气2($year, $month) {
		$lunarHoliDayName = array( 
			'小寒', '大寒', 
			'立春', '雨水', '惊蛰', '春分', '清明', '谷雨', 
			'立夏', '小满', '芒种', '夏至', '小暑', '大暑', 
			'立秋', '处暑', '白露', '秋分', '寒露', '霜降', 
		...
		for ($n = $month * 2 - 1; $n <= $month * 2; $n++) {
			$st = new 节气();
			$dd = self::term($year, $n, true);
			$sd1 = LunarUtils::antiDayDifference($year, floor($dd));
			// $sm1 = floor(sd1 / 100);
			$h = floor(LunarUtils::tail($dd) * 24);
			$min = floor((LunarUtils::tail($dd) * 24 - $h) * 60);
			$mmonth = ceil($n / 2);
			$day = $sd1 % 100;
			$st->setSolarTermDate(Date::get($year, $mmonth, $day, $h, $min, 0));
	 ...
	 * @throws Exception
	 */
	public static function 节气($solarDateTime) {
		return self::节气2($solarDateTime->getYear(), $solarDateTime->getMonth());
	}
	 ...
	 * @return
	 */
	public static function term($y, $n, $pd) {
		$juD = $y
		...
		$shuoD = -18e-4
				* sin(2.313908653 * $y - 0.439822951 - 3.0443 * $n);// 朔差实均数
		$vs = ($pd) ? 
			($juD + $yrD + $shuoD - LunarUtils::equivalentStandardDay($y, 1, 0) - 1721425) : 
			($juD - LunarUtils::equivalentStandardDay($y, 1, 0) - 1721425);
		return $vs;
	}
}
